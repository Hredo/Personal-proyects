...
async function main() {
  ...
  for (const l of allLeagues) {
    ...
    const noLogo = await db
      .select({ c: sql`count(*)` })
      .from(teams)
      .where(and(eq(teams.leagueId, leagueId), isNull(teams.logoUrl)))
    const noCity = await db
      .select({ c: sql`count(*)` })
      .from(teams)
      .where(and(eq(teams.leagueId, leagueId), isNull(teams.city)))
    const noArena = await db
      .select({ c: sql`count(*)` })
      .from(teams)
      .where(and(eq(teams.leagueId, leagueId), isNull(teams.arena)))
    console.log(
      `  ${l.name}: ${total[0].c} teams, ${noLogo[0].c} no logo, ${noCity[0].c} no city, ${noArena[0].c} no arena`,
    )
  }
