...
function categoryScoring(a: ComparePlayer, b: ComparePlayer): CategoryResult {
  ...
  return {
    ...
    summary: cmp.winner === "tie" || cmp.winner === "n/a"
...
function categoryPlaymaking(a: ComparePlayer, b: ComparePlayer): CategoryResult {
...
function categoryAvailability(
  ...
  const aLoad =
    aMin != null && aGp != null ? aMin * aGp : (aMin ?? aGp);
  const bLoad =
    bMin != null && bGp != null ? bMin * bGp : (bMin ?? bGp);
...
function buildFitNotes(a: ComparePlayer, b: ComparePlayer): string[] {
  ...
  if (aArch.includes("Anotador") && bArch.includes("director")) {
    notes.push(`Combo ofensivo natural: ${a.fullName} crea su tiro y ${b.fullName} le organiza el juego.`);
  } else if (bArch.includes("Anotador") && aArch.includes("director")) {
    notes.push(`Combo ofensivo natural: ${b.fullName} crea su tiro y ${a.fullName} le organiza el juego.`);
  }

  if (aArch.includes("defensivo") || bArch.includes("defensivo")) {
    notes.push("Al menos uno aporta un perfil defensivo claro — útil para cerrar partidos en defensa.");
  }

  if (aArch.includes("tirador") && bArch.includes("tirador")) {
    notes.push("Ambos abren el campo desde el perímetro: alineaciones muy espaciadas pero con menos creación interior.");
  }

  if (a.position && b.position && a.position[0] === b.position[0]) {
    notes.push(`Comparten posición base (${a.position}/${b.position}). Si juegan juntos, exige un sistema con dobles bases o ala-pívots versátiles.`);
  }

  if (notes.length === 0) {
    notes.push("Perfiles complementarios en muchas áreas — fácil de combinar en rotación.");
  }
  return notes;
}
...
function buildWarnings(a: ComparePlayer, b: ComparePlayer): string[] {
  ...
  if (!a.stats) warnings.push(`No hay stats de temporada para ${a.fullName}; el análisis se apoya en datos parciales.`);
  if (!b.stats) warnings.push(`No hay stats de temporada para ${b.fullName}; el análisis se apoya en datos parciales.`);
  if (a.league.slug !== b.league.slug) {
    warnings.push("Comparar entre ligas distintas no normaliza pace ni nivel defensivo. Toma los rankings como orientativos.");
  }
  const aGp = num(a.stats?.gamesPlayed ?? null) ?? 0;
  const bGp = num(b.stats?.gamesPlayed ?? null) ?? 0;
  if (aGp > 0 && aGp < 15) warnings.push(`Muestra pequeña para ${a.fullName} (${aGp} GP).`);
  if (bGp > 0 && bGp < 15) warnings.push(`Muestra pequeña para ${b.fullName} (${bGp} GP).`);
  return warnings;
}
