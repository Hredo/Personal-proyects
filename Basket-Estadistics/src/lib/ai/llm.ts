...
function buildPlayerContext(profile: PlayerProfile): string {
  ...
  if (profile.heightCm) lines.push(`- Altura: ${(profile.heightCm / 100).toFixed(2)} m`);
  ...
  if (latest) {
    ...
    if (latest.gamesPlayed !== null) lines.push(`- Partidos: ${latest.gamesPlayed}`);
    if (latest.minutesPerGame !== null)
      lines.push(`- Minutos por partido: ${formatStat(latest.minutesPerGame)}`);
    if (latest.points !== null) lines.push(`- Puntos: ${formatStat(latest.points)} PPG`);
    if (latest.rebounds !== null) lines.push(`- Rebotes: ${formatStat(latest.rebounds)} RPG`);
    if (latest.assists !== null) lines.push(`- Asistencias: ${formatStat(latest.assists)} APG`);
    if (latest.steals !== null) lines.push(`- Robos: ${formatStat(latest.steals)} SPG`);
    if (latest.blocks !== null) lines.push(`- Tapones: ${formatStat(latest.blocks)} BPG`);
    if (latest.turnovers !== null) lines.push(`- Pérdidas: ${formatStat(latest.turnovers)} TOPG`);
...
function buildTeamContext(team: TeamProfile): string {
  ...
  if (team.seasonStats) {
    ...
    if (team.seasonStats.netRtg !== null) lines.push(`- Net Rating: ${team.seasonStats.netRtg.toFixed(1)}`);
  } else {
