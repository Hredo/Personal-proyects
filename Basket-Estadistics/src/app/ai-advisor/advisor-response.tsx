...
export function AdvisorResponse({ data }: { data: AdvisorOutput }) {
  return (
    <div className="space-y-4 w-full">
      ...
      >
        <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${LEAGUE_COLORS[data.team.leagueBadge] || "from-brand-500 to-brand-300"}`} />
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-2xl">{data.intentEmoji}</span>
              <span className="text-[10px] uppercase tracking-widest text-ink-400">Análisis para</span>
            </div>
            <h2 className="text-xl font-bold text-ink-50">{data.team.name}</h2>
          </div>
          <div className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${LEAGUE_BG[data.team.leagueBadge]}`}>
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-ink-300">
          <span className="rounded-md bg-ink-700/40 px-2 py-1">
            <span className="text-ink-500">Récord</span> · <span className="font-mono font-semibold text-ink-100">{data.team.record}</span>
          </span>
          <span className="rounded-md bg-ink-700/40 px-2 py-1">
            <span className="text-ink-500">Plantilla</span> · <span className="font-mono font-semibold text-ink-100">{data.team.rosterSize} jug</span>
          </span>
        </div>
      </motion.div>
      ...
      >
        <div className="mb-2 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-ink-100">Diagnóstico del equipo</h3>
        </div>
        ...
        <div className="mt-3 rounded-lg border-l-2 border-brand-500/60 bg-brand-500/5 px-3 py-2 text-xs text-ink-200">
          <span className="font-semibold text-brand-300">Hueco detectado: </span>{data.gap}
        </div>
        {data.team.topPlayers.length > 0 && (
          <div className="mt-3 flex flex-wrap items-center gap-1.5 text-[11px]">
            <span className="text-ink-500">Núcleo actual:</span>
            {data.team.topPlayers.map((p) => (
              <span key={p} className="rounded-md bg-ink-700/30 px-1.5 py-0.5 text-ink-300">{p}</span>
            ))}
          </div>
        )}
      </motion.div>
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <h3 className="text-sm font-semibold text-ink-100">Candidatos recomendados</h3>
        </motion.div>
      ...
      >
        <div className="mb-2 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-ink-200">Antes de negociar</h3>
        </div>
...
function RecruitCard({ rec, index }: { rec: Recruit & { priority: string; priorityColor: string }; index: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 pl-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${LEAGUE_BG[rec.league] || "bg-ink-700/40 text-ink-300 border-ink-600"}`}>
              {rec.league}
            </span>
          </div>
        ...
        <div className="shrink-0 text-right">
          <div className="font-mono text-sm font-bold text-ink-100">{rec.contractValue}</div>
          <div className="mt-0.5 text-[10px] uppercase tracking-wider text-ink-500">{rec.market}</div>
        </div>
      </div>

      <div className="mt-3 pl-2">
        <div className={`inline-block rounded-full border px-2 py-0.5 text-[10px] font-semibold ${rec.priorityColor}`}>
          {rec.priority}
        </div>
      </div>
      ...
      <div className="mt-3 pl-2">
        <p className="text-sm leading-relaxed text-ink-200">
          <span className="text-brand-300 font-semibold">Encaje: </span>{rec.fit}
        </p>
      </div>

      <div className="mt-3 flex flex-wrap gap-1.5 pl-2">
        {rec.strengths.map((s) => (
          <span key={s} className="rounded-md bg-ink-700/30 px-2 py-0.5 text-[11px] text-ink-300">
            {s}
          </span>
        ))}
      </div>
