...
export function TeamSelector({ onTeamChange }: { onTeamChange: (team: TeamOption) => void }) {
  ...
  const handleSelect = useCallback((team: TeamOption) => {
    setSelected(team);
    setQuery(team.name);
    setOpen(false);
    onTeamChange(team);
    ...
    <div className="w-full px-4 py-3" ref={wrapperRef}>
      ...
      <div className="relative">
        <input
          ...
          onKeyDown={onKeyDown}
          placeholder={loading ? "Cargando equipos..." : "Buscar equipo (NBA, EuroLeague, ACB)..."}
          disabled={loading}
        ...
        {open && (
          <div className="absolute z-20 mt-1 w-full rounded-lg border border-ink-700 bg-ink-800 shadow-xl max-h-56 overflow-y-auto">
            {filtered.length === 0 ? (
