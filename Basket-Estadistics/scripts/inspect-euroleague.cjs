...
const teams = get("SELECT id, slug, name, league_id, logo_url, primary_color, city, arena FROM teams WHERE league_id = ? ORDER BY name", [el.id])
...
console.log('Sample slugs:', teams.slice(0, 5).map(t => t.slug).join(', '))

const players = get("SELECT id, slug, full_name, photo_url, position, nationality, birthdate, height_cm, weight_kg, current_team_id FROM players WHERE source = 'euroleague'")
console.log(`\n=== EuroLeague players (${players.length}) ===`)
const pMissing = { photo: 0, position: 0, nationality: 0, birthdate: 0, height: 0, weight: 0, currentTeam: 0 }
...
const coaches = get("SELECT full_name, role, photo_url, nationality, age FROM coaches WHERE league_id = ?", [el.id])
...
const tStats = get1("SELECT COUNT(*) as total FROM team_season_stats ts JOIN seasons s ON s.id = ts.season_id WHERE s.league_id = ?", [el.id])
console.log(`\n=== EuroLeague team_season_stats rows: ${tStats.total} ===`)

const seasons = get("SELECT * FROM seasons WHERE league_id = ? ORDER BY year DESC", [el.id])
